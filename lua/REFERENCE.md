...
```lua
local results, err = client:Giveaway(nil):list(nil, nil)
```
...
```lua
local result, err = client:Giveaway(nil):load({ id = "giveaway_id" }, nil)
```
...
```lua
local result, err = client:Worth(nil):load({ id = "worth_id" }, nil)
```
