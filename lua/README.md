...
The Lua SDK for the FreeGames API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Giveaway()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/free-games-sdk/releases)),
...
| `id` | `number` | Unique identifier for the giveaway |
...
| `users` | `number` | Number of users participating in the giveaway |
...
| Field | Type | Description |
| --- | --- | --- |
| `active_giveaways_number` | `number` | Number of active giveaways |
| `worth_estimation_usd` | `string` | Total estimated worth in USD |

#### Example: Load

```lua
local worth, err = client:Worth():load()
```
...
```
lua/
├── free-games_sdk.lua    -- Main SDK module
...
The main module (`free-games_sdk`) exports the SDK constructor
