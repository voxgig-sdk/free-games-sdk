...
local GiveawayEntity = {}
GiveawayEntity.__index = GiveawayEntity


function GiveawayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GiveawayEntity)
  self._name = "giveaway"
  self._client = client
...
function GiveawayEntity:get_name()
  return self._name
end


function GiveawayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GiveawayEntity.new(self._client, opts)
end
...
function GiveawayEntity:mark_deleted()
  self._deleted = true
end


function GiveawayEntity:deleted()
  return true == self._deleted
end


function GiveawayEntity:data_set(args)
  if args ~= nil then
...
function GiveawayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GiveawayEntity:match_set(args)
  if args ~= nil then
...
function GiveawayEntity:match_get()
...
function GiveawayEntity:stream(action, args, callopts)
...
function GiveawayEntity:_run_op(ctx, post_done)
  local utility = self._utility
