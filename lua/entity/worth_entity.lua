...
local WorthEntity = {}
WorthEntity.__index = WorthEntity


function WorthEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorthEntity)
  self._name = "worth"
  self._client = client
...
function WorthEntity:get_name()
  return self._name
end


function WorthEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorthEntity.new(self._client, opts)
end
...
function WorthEntity:mark_deleted()
  self._deleted = true
end


function WorthEntity:deleted()
  return true == self._deleted
end


function WorthEntity:data_set(args)
  if args ~= nil then
...
function WorthEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorthEntity:match_set(args)
  if args ~= nil then
...
function WorthEntity:match_get()
...
function WorthEntity:stream(action, args, callopts)
...
---@param reqmatch WorthLoadMatch
---@param ctrl? table
---@return Worth
---@return string? err
function WorthEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WorthEntity:_run_op(ctx, post_done)
  local utility = self._utility
