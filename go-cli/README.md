...
boru-driven command-line client **and** interactive REPL for the FreeGames
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-games-cli)
make build
...
./free-games-cli list giveaway
./free-games-cli load 1 giveaway            # {id:1} shorthand
./free-games-cli load '{id:1}' giveaway       # explicit match map
./free-games-cli list worth

# 5. Override the API base URL for a single call
FREE_GAMES_BASE=https://api.example.com ./free-games-cli list giveaway

# 6. No arguments -> interactive REPL
./free-games-cli
free-games> list giveaway
free-games> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-games-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-games-cli list giveaway
   ```
...
```sh
./free-games-cli load 1 giveaway          # scalar shorthand for {id:1}
./free-games-cli load '{id:1}' giveaway     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `free-games>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `giveaway`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-games-cli`. |
...
(`./free-games-cli <expr>`) and interactively (the REPL), and expressions compose the
