...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.FREEGAMES_TEST_LIVE ||
    'TRUE' === process.env.FREEGAMES_TEST_OVERRIDE
  ) {
  ...
  m.FREEGAMES_TEST_EXPLAIN = process.env.FREEGAMES_TEST_EXPLAIN || m.FREEGAMES_TEST_EXPLAIN
