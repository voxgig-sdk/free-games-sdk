...
//     (\`new FreeGamesSDK(...)\` / \`FreeGamesSDK.test(...)\` become
//     \`FreeGamesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"giveaway":{"test01":{"id":"test01"}},"worth":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeGamesSDK(...)\` and any
// \`FreeGamesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeGamesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeGamesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
