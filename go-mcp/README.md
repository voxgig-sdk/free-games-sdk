...
[MCP](https://modelcontextprotocol.io) server exposing the FreeGames SDK as
two agent tools — `free-games_list` and `free-games_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-games-mcp)
make build

# 2. Provide credentials via the environment
export FREE_GAMES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-games \
  -- /absolute/path/to/free-games-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-games-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-games_list: first page of records
{ "entity": "giveaway" }
{ "entity": "giveaway", "query": { } }

// free-games_load: one record by id
{ "entity": "giveaway", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-games-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-games \
     -- "$PWD"/dist/*/free-games-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-games_list` and `free-games_load` tools now appear
   in new sessions. Ask the agent to *"list giveaway using free-games"*
   and it calls `free-games_list` with `{"entity":"giveaway"}`.
...
```sh
export FREE_GAMES_APIKEY=sk_live_xxx            # API key
export FREE_GAMES_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-games_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-games_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_GAMES_APIKEY` | API key sent with every request. |
| `FREE_GAMES_BASE` | Optional override of the API base URL. |
...
```sh
./free-games-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-games_load","arguments":{"entity":"giveaway","query":{"id":1}}}}'
```
