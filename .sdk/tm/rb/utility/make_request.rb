...
module FreeGamesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeGamesResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeGamesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeGamesResponse.new(fetched)
    else
