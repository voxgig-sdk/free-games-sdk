...
class FreeGamesMakeError
{
    public static function call(?FreeGamesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeGamesContext([], null);
        }
        $op = $ctx->op ?? new FreeGamesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeGamesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeGamesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeGamesError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FreeGamesError) {
