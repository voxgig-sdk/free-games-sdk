...
class FtCtrl extends FreeGamesControl
{
...
class FtClient
{
    ...
    private FreeGamesUtility $utility;

    public function __construct(FreeGamesUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeGamesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeGamesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeGamesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeGamesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeGamesConfig::make_feature($name);
    ...
    private static function buildUrl(FreeGamesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeGamesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeGamesResponse($fetched);
        }
    ...
    private function fail(FreeGamesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeGamesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
