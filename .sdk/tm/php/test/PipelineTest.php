...
{
    private static function utility(): FreeGamesUtility
    {
        return new FreeGamesUtility();
    }
    ...
    private static function ctx(array $over = []): FreeGamesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new FreeGamesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new FreeGamesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof FreeGamesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new FreeGamesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = FreeGamesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new FreeGamesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = FreeGamesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new FreeGamesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = FreeGamesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeGamesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = FreeGamesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new FreeGamesResponse([]),
            'result' => new FreeGamesResult([]),
        ]);
        [, $err] = FreeGamesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new FreeGamesSpec([]),
            'response' => null,
            'result' => new FreeGamesResult([]),
        ]);
        [, $err] = FreeGamesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new FreeGamesSpec(['step' => 's']),
            'response' => new FreeGamesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new FreeGamesResult([]),
        ]);
        [, $err] = FreeGamesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeGamesSpec(['step' => 's']),
            'response' => new FreeGamesResponse(self::resp(200, ['v' => 1])),
            'result' => new FreeGamesResult([]),
        ]);
        [, $err] = FreeGamesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new FreeGamesSpec(['step' => 's']),
            'response' => new FreeGamesResponse(self::resp(200, ['v' => 2])),
            'result' => new FreeGamesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        FreeGamesMakeResponse::call($ctx);
    ...
    {
        $preset = new FreeGamesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new FreeGamesSpec([]),
            'response' => new FreeGamesResponse([]),
            'result' => new FreeGamesResult([]),
        ...
        [$got, $err] = FreeGamesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new FreeGamesResult([])]);
        [, $err] = FreeGamesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new FreeGamesSpec([]), 'result' => null]);
        [, $err] = FreeGamesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeGamesSpec(['step' => 's']),
            'result' => new FreeGamesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new FreeGamesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeGamesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new FreeGamesSpec(['step' => 's']),
            'result' => new FreeGamesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new FreeGamesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = FreeGamesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new FreeGamesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new FreeGamesSpec([]),
            'result' => new FreeGamesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = FreeGamesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeGamesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new FreeGamesError('boom', 'boom');
        $utility->fetcher = function (FreeGamesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeGamesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = FreeGamesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (FreeGamesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = FreeGamesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(FreeGamesResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (FreeGamesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new FreeGamesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = FreeGamesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new FreeGamesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new FreeGamesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = FreeGamesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = FreeGamesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = FreeGamesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new FreeGamesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, FreeGamesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new FreeGamesResult(['ok' => false])]);
        $this->expectException(FreeGamesError::class);
        FreeGamesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new FreeGamesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', FreeGamesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        FreeGamesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new FreeGamesBaseFeature();
        $b = new FreeGamesBaseFeature();
        FreeGamesFeatureAdd::call($ctx, $a);
        FreeGamesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): FreeGamesBaseFeature
    {
        $f = new FreeGamesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        FreeGamesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): FreeGamesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new FreeGamesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = FreeGamesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new FreeGamesResponse(['headers' => null]),
            'result' => new FreeGamesResult([]),
        ]);
        FreeGamesResultHeaders::call($ctx);
