...
class GiveawayEntity
  ...
  def make
    opts = @_entopts.dup
    GiveawayEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeGamesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Giveaway, Hash] the current Giveaway data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FreeGamesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Giveaway fields)
  def match_get
  ...
  #
  # @param reqmatch [GiveawayLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Giveaway.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Giveaway, Hash] the loaded Giveaway; raises FreeGamesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeGamesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [GiveawayListMatch, Hash, nil] match filter (any subset of
  #   Giveaway fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Giveaway>, Array] the matching Giveaway items; raises FreeGamesError on failure
