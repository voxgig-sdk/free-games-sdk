...
require_relative 'FreeGames_types'
...
class FreeGamesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeGamesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeGamesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeGamesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeGamesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeGamesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeGamesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeGamesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FreeGamesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeGamesError.new(
        "graphql_error", "FreeGamesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
