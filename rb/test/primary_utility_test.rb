...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FreeGamesOmni.make_runner(TEST_JSON_FILE, FreeGamesSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FreeGamesOmni.errify(err)
    end
