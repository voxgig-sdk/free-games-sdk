...
The Ruby SDK for the FreeGames API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Giveaway(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.FreeGames(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── FreeGames_sdk.rb       -- Main SDK module
