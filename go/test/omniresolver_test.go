...
import (
	...
	omni "github.com/voxgig-sdk/free-games-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeGamesError); is {
				out := map[string]any{
					"name":    "FreeGamesError",
...
func omniCtx(arg any, client *sdk.FreeGamesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeGamesSDK); is {
			client = live
