...
//
// NewFreeGamesSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.FreeGamesSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewFreeGamesSDK(opts)
}
...
func fcCandidates(client *sdk.FreeGamesSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.FreeGamesSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.FreeGamesSDK, name string) any {
