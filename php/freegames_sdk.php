...
class FreeGamesSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = FreeGamesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = FreeGamesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!FreeGamesFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, FreeGamesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return FreeGamesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = FreeGamesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = FreeGamesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = FreeGamesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new FreeGamesError($op . "_allow",
                "FreeGamesSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = FreeGamesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new FreeGamesError("graphql_error",
                "FreeGamesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Giveaway($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_giveaway === null) {
                $this->_giveaway = new GiveawayEntity($this, null);
            }
            return $this->_giveaway;
        }
        return new GiveawayEntity($this, $data);
    }
    ...
    public function Worth($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_worth === null) {
                $this->_worth = new WorthEntity($this, null);
            }
            return $this->_worth;
        }
        return new WorthEntity($this, $data);
    }
