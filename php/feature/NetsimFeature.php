...
class FreeGamesNetsimFeature extends FreeGamesBaseFeature
{
    ...
    private function _simulate(FreeGamesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(FreeGamesContext $ctx, array $applied): void
    {
        $client = $this->client;
