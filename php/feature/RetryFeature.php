...
class FreeGamesRetryFeature extends FreeGamesBaseFeature
{
    ...
    public function init(FreeGamesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeGamesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeGamesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeGamesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
