...
The PHP SDK for the FreeGames API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Giveaway(null)->list(null, null);
...
```php
[$result, $err] = $client->Giveaway(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->FreeGames(null)->load(
    ["id" => "test01"], null
);
