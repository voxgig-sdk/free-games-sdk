...
The PHP SDK for the FreeGames API — an entity-oriented client using PHP conventions.
...
## Install
```bash
composer require voxgig-sdk/free-games
```
...
```php
[$result, $err] = $client->Giveaway()->list();
...
```php
[$result, $err] = $client->Giveaway()->load(["id" => "example_id"]);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->FreeGames()->load(["id" => "test01"]);
...
```
php/
├── freegames_sdk.php          -- Main SDK class
...
The main class (`freegames_sdk.php`) exports the SDK class
