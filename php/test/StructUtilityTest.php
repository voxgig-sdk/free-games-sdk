...
// FreeGames SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = FreeGamesOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(FreeGamesOmni::structfix($now, Struct::undef()))
                        !== json_encode(FreeGamesOmni::structfix($val, Struct::undef()))) {
