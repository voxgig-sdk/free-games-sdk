...
The Python SDK for the FreeGames API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Giveaway(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Giveaway(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.FreeGames(None).load(
    {"id": "test01"}, None
)
