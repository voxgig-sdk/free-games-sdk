...
class FreeGamesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeGamesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeGamesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeGamesTestRunner.getenv("FREE_GAMES_TEST_LIVE")
        override = FreeGamesTestRunner.getenv("FREE_GAMES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeGamesTestRunner.getenv(key)
        ...
        explain = FreeGamesTestRunner.getenv("FREE_GAMES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["FREE_GAMES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if FreeGamesTestRunner._test_control is not None:
            return FreeGamesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeGamesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeGamesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeGamesTestRunner.load_test_control()
...
def load_env_local():
    FreeGamesTestRunner.load_env_local()


def env_override(m):
    return FreeGamesTestRunner.env_override(m)


def entity_data(v):
    return FreeGamesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeGamesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeGamesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeGamesTestRunner.load_test_control()


def live_client_options():
    return FreeGamesTestRunner.live_client_options()


def live_delay_ms():
    return FreeGamesTestRunner.live_delay_ms()
