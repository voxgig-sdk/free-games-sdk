...
from freegames_sdk.core.spec import FreeGamesSpec
from freegames_sdk.core.result import FreeGamesResult
from freegames_sdk.core.response import FreeGamesResponse
from freegames_sdk.core.operation import FreeGamesOperation
from freegames_sdk.core.error import FreeGamesError
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return FreeGamesError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = FreeGamesSDK.test(None, None)
        ...
        class TestInitFeature(FreeGamesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(FreeGamesBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = FreeGamesSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = FreeGamesSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = FreeGamesSDK.test(None, setup_opts)
